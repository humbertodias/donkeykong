...
void CApp::OnLoop() {
  ...
  for (int i = 0; i < entityList.size();
       i++) {                // loops through all entities (including mario)
  ...
  mario.setLadderCollide(
      0); // reset to false each time through, will both be checked and set in
  ...
  for (int i = 0; i < entityList.size(); i++) {
    if (entityList[i]->IsCollision(mario) == 1 &&
        mario.getState() != MARIO_HURTING) {
      ...
      } else if (entityList[i]->identifyType() == 2 ||
                 entityList[i]->identifyType() == 3) { // fire or dk object
        Mix_PlayChannel(-1, hurts, 0); // Play hurting sound effect
      }
    } else if (entityList[i]->IsCollision(mario) == 2 &&
               mario.getState() == MARIO_JUMPING && gotPoints == 0) {
      gotPoints++;
    }
  }
  ...
  if (peach.IsCollision(mario)) {
    ...
    Surf_Points = TTF_RenderText_Solid(pointsFont, "+1500",
                                       {255, 105, 180}); // pink for peach
  ...
  if (gotPoints > 0) {
    ...
    if (gotPoints > 8 &&
        mario.getState() !=
            MARIO_HURTING) { // successfully jumped over fire or barrel (ie. did
                             // not fall onto it)
      gotPoints = 0;
      score += 200;
      ...
      Surf_Points = TTF_RenderText_Solid(
          pointsFont, "+200",
          {255, 100, 70}); // orange for scoring over barrels and fire
  ...
  if (dyingCount > 0) {
    ...
    if (dyingCount > 50) {
      ...
      if (mario.getLives() ==
          0) { // game is over when mario has no remaining lives
        running = 0;
        wonLevel1 = 0;
        wonLevel2 = 0;
      } else {
