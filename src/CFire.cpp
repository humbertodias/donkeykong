...
void CFire::OnLoop() {
  ...
  if ((y < 0) ||
      (y + FIRE_HEIGHT > WINDOW_HEIGHT)) { // If FIRE moves too far up or down
...
void CFire::OnRender(SDL_Surface *Surf_Display) {
  if (frame >= 3)
    frame = 0;
  CSurface::OnDraw(Surf_Display, Surf_Entity, x, y, clips[frame].x,
                   clips[frame].y, FIRE_WIDTH, FIRE_HEIGHT);
}

int CFire::identifyType() { return 1; }
...
int CFire::IsDiffLevel(CMario &mario) {
  ...
  if (this->y - mario.getY() >
      22) { // 22 was found to be the best distance to determine if fire & mario
            // are on diff levels
    return 1;
  } else {
    return 0;
...
int CFire::wheresMarioX(CMario &mario) {
  if ((mario.getX() - 15 > this->x) &&
      (this->xVel < 0)) { // 15 is used as buffer to make the AI more forgiving
    this->xVel *= -1;
...
int CFire::getState() { return state; }

void CFire::setState(fireState _state) { state = _state; }
...
void CFire::setXinit(int _x) { xinit = _x; }

void CFire::setYinit(int _y) { yinit = _y; }
