...
#include "CPeach.h"
#define SLOWED                                                                 \
  1 // set to 1 if want to slow peach by half, otherwise peach moves too quickly
...
void CPeach::OnLoop() {
  ...
  if (!moved) {
    ...
    frame++;
    if (frame > 1)
      frame = 0; // only 2 frames per direction
...
void CPeach::OnRender(SDL_Surface *Surf_Display) {
  if (direction == -1) { // left moving peach
    CSurface::OnDraw(Surf_Display, Surf_Entity, x, y, frame * PEACH_WIDTH, 0,
                     PEACH_WIDTH, PEACH_HEIGHT);
  } else { // right moving peach
    CSurface::OnDraw(Surf_Display, Surf_Entity, x, y, (2 + frame) * PEACH_WIDTH,
                     0, PEACH_WIDTH, PEACH_HEIGHT);
  }
}

int CPeach::IsCollision(CMario &mario) {
  // returns true upon collision with CMario object, indicates winning of level
  if ((mario.getX() < this->x + this->width + 5) &&
      (mario.getX() + mario.getWidth() > this->x) &&
      (mario.getY() < this->y + this->height) &&
...
int CPeach::identifyType() { return 4; }
