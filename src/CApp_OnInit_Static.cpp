...
int CApp::OnInit_Static() {
    ...
    Surf_bgObjs = CSurface::OnLoad("res/img/bg/staticObjects.png");  // load file containing static objects
                                                                     // image sprites
    if (Surf_bgObjs == NULL) {
    ...
    for (i = 0; i < 7; i++) {  // initializes the 7 platforms on each level (1-4)
        temp.OnInit(baseXLOdd - xIncrement * i, baseYL1 - yIncrement * i, PLATFORM_WIDTH,
                    PLATFORM_HEIGHT);  // level 1 initialization
        platforms.push_back(temp);
