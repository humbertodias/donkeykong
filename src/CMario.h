...
enum marState {  // values for state private data member
  ...
  MARIO_JUMPING, // state of mario during a jump (in the air)-- disable SPACE
                 // arrow key event
  MARIO_CLIMBING, // state of mario while climbing a ladder -- disable L R arrow
                  // key events
...
public:
  ...
  CMario
  operator++(); // overloaded pre-increment, indicates mario got an extra life
  CMario
  operator--(); // overloaded pre-decrement, indicates loss of life for mario
  int getLives();
  void setLives(int);
...
private:
  ...
  int ladderCollide;   // true when mario is currently colliding with a ladder
