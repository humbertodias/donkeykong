 ...
 * DonkeyKong
 * StaticObj.cpp
 * This file contains the implementation for the StaticObj class, an abstract
 * class of objects that make up the static framework of the game.
 */
...
void StaticObj::OnRender(SDL_Surface *Surf_Display, SDL_Surface *Surf_ImgFile) {
  if (Surf_Display != NULL && Surf_ImgFile != NULL) {
    CSurface::OnDraw(Surf_Display, Surf_ImgFile, x, y, xBlit, yBlit, width,
                     height);
  }
}
