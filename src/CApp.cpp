 ...
 * DonkeyKong
 * CApp.cpp
 * This file contains implementation of OnExecute() function of CApp class, as
 * well as the main function for the application.  OnExecute() contains the
...
void CApp::resetLevel() { // called whenever a level is started or after mario
                          // loses a life in a level
  running = 1;
  mario.reset();
  barrel.reset();
  barrel2.reset();
  ...
  while (entityList.size() > constantEntitiesCount) {
    entityList.pop_back(); // removes additional barrels from entityList to be
                           // readded during the level play
  }
  fire.reset();
  fire2.reset();
  fire3.reset();
...
int CApp::OnExecute() {
  ...
  if (Mix_PlayMusic(theme, -1) ==
      -1) { // Play background music -- smb3 theme song
    return 1;
  }
  SDL_Event Event;
  ...
  while (game) {
    ...
    while (running) {
      ...
      while (SDL_PollEvent(
          &Event)) { // use while to go through any events on a queue one at a
                     // time, SDL_PollEvent returns 0 when no events on a queue
        if (!OnEvent(&Event)) {
          OnCleanup();
          return 1;
        }
    ...
    if (wonLevel1) {     // start level 2
      ...
      SDL_FillRect(Surf_Display, &Surf_Display->clip_rect,
                   SDL_MapRGB(Surf_Display->format, 0, 0, 0));
      Surf_LevelText =
          TTF_RenderText_Solid(largeFont, "LEVEL 2", {255, 255, 0});
      CSurface::OnDraw(Surf_Display, Surf_LevelText, 200, 225);
      SDL_Flip(Surf_Display);
      SDL_Delay(1500);
      //////play LEVEL 2//////
      while (running) {
        ...
        while (SDL_PollEvent(
            &Event)) { // use while to go through any events on a queue one at a
                       // time, SDL_PollEvent returns 0 when no events on queue
          if (!OnEvent(&Event)) {
            OnCleanup();
            return 1;
          }
    ...
    if (wonLevel2) {
      Surf_LevelText =
          TTF_RenderText_Solid(largeFont, "YOU WIN!", {255, 0, 125});
    } else {
      Surf_LevelText =
          TTF_RenderText_Solid(largeFont, "YOU LOSE.", {255, 0, 0});
    }
