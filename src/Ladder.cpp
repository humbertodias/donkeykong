...
void Ladder::IsCollision(CMario &mario) {
  ...
  if ((mario.getX() > this->x) &&
      (mario.getX() + mario.getWidth() < this->x + this->width) &&
    ...
    if ((this->height > 30) ||
        (mario.getY() >
         this->y - mario.getHeight() /
                       2)) { // can climb above ladder to half of his height
      mario.setLadderCollide(
          1); // ladderCollide set to true, know when exit ladder
    }
  }
}
