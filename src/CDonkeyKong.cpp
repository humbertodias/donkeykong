...
void CDonkeyKong::OnRender(SDL_Surface *Surf_Display) {
  if (frame <= 15) {
    CSurface::OnDraw(Surf_Display, Surf_Entity, x, y, frame * DK_WIDTH, 0,
                     DK_WIDTH, DK_HEIGHT);
  } else { // Adjusts for the fact that the sprite gets off-center
    CSurface::OnDraw(Surf_Display, Surf_Entity, x, y, frame * (DK_WIDTH + 1), 0,
                     DK_WIDTH, DK_HEIGHT);
  }
}

int CDonkeyKong::identifyType() { return 3; }
