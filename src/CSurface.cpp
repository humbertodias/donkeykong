...
int CSurface::OnDraw(SDL_Surface* Surf_Dest, SDL_Surface* Surf_Src, int x,
                     int y) {  //(x,y) location of placement of Surf_Src on Surf_Dest
    if (Surf_Dest == NULL || Surf_Src == NULL) {
        return 0;  // must have a valid source and destination surfaces for blitting
    }

    SDL_Rect DestR;  // data members- x, y, w, h ; only concerned with x, y (where
                     // drawing, offsets)
    DestR.x = x;
    DestR.y = y;
...
int CSurface::OnDraw(SDL_Surface* Surf_Dest, SDL_Surface* Surf_Src, int x, int y, int x2, int y2, int w, int h) {
    ...
    SDL_Rect SrcR;  // specify the pixels of the source file to blit to the
                    // destination file
    SrcR.x = x2;
    SrcR.y = y2;
    SrcR.w = w;
...
int CSurface::Transparent(SDL_Surface* Surf_Dest, int r, int g, int b) {
    ...
    SDL_SetColorKey(Surf_Dest, SDL_SRCCOLORKEY | SDL_RLEACCEL,
                    SDL_MapRGB(Surf_Dest->format, r, g,
                               b));  // rgb correspond to color to make transparent
