 ...
 * DonkeyKong
 * CApp_OnRender.cpp
 * This file contains the implementation of the OnRender() function of class
 * CApp. This function renders all Surfaces to Surf_Display)
 */
...
void CApp::OnRender() {
  ...
  for (i = 0; i < entityList.size();
       i++) { // renders each valid entity to Surf_Display data member of CApp
    if (!entityList[i])
      continue;
  ...
  if (displayPoints > 0) { // true when mario has just earned points, display
                           // the +200/+1500 when gain points
