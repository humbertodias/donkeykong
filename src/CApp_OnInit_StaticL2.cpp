 ...
 *	This file sets up ladder and platform objects for a second level,
 *implemented in a similar way as OnInit_Static() for the first level.
 */
...
int CApp::OnInit_StaticL2() {
  ...
  Surf_bgObjs = CSurface::OnLoad(
      "res/img/bg/staticObjectsL2.png"); // load file containing static objects
                                         // image sprites
  if (Surf_bgObjs == NULL) {
  ...
  // begin 0th level
  for (i = 0; i < 8; i++) {
    temp.OnInit(420 - xIncrement * i, LEVEL_0_MIN - yIncrement * i,
  ...
  // begin 1st level
  temp.OnInit(420, 510 - yIncrement - PLATFORM_HEIGHT, PLATFORM_WIDTH,
              PLATFORM_HEIGHT);
  platforms.push_back(temp);
